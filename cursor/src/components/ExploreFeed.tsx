  ...
  return (
    <div>
      <div className="sticky top-0 z-10 bg-[#0f0f0f]/95 backdrop-blur border-b border-white/10 px-4 py-3">
        ...
        <p className="text-gray-500 text-sm mt-0.5">Posts em destaque por curtidas e retweets</p>
      </div>
      {loading ? (
        <div className="p-8 text-center text-gray-500">Carregando...</div>
      ) : posts.length === 0 ? (
        <div className="p-8 text-center text-gray-500">Nenhum post para explorar ainda.</div>
      ) : (
        <>
          {posts.map((post) => (
